 ...
 * @time 2016-06-02
 * */
public class CalculationSphereEarth {
    /**
     * 地球赤道半径(km)
     * */
	...
	public static Double[] getRealPosition(double x, double y, double lon, double lat, double angle) {
		...
		if (brng < 0)
			brng = brng + 360;
		double realAngle=(brng+angle)%360;//与正北的夹角
		double distance=Math.sqrt(Math.pow(x, 2)+Math.pow(y, 2));
		String lonLat = ConvertDistanceToLogLat(lon,lat,distance,realAngle);
		String[] lls = lonLat.split(",");
    	return new Double[]{new Double(lls[0]),new Double(lls[1])};
    }
...
//                + ConvertDistanceToLogLat(lon1, lat1, distance, azimuth) + ")");
        ...
        double centerLon=123.485273;
        double centerLat=41.638819;
        
        //String convertDistanceToLogLat = CalculationLogLatDistance.ConvertDistanceToLogLat(centerLon, centerLat, 1001.18, 89.78);
        //System.out.println("------------->"+convertDistanceToLogLat);
        
       // String getOtherPoint = CalculationLogLatDistance.GetOtherPoint(centerLon, centerLat, 1001.18, 89.78);
        //System.out.println(getOtherPoint);

        Double[] realPosition = CalculationSphereEarth.getRealPosition(-50d, 50d, centerLon, centerLat, 90d);
